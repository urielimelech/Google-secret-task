...
def solution(data, n):
    dictonary = {}
    delete_indexes = []
    if n <= 0 or len(data) > 99: 
        return []
    for index, number in enumerate(data):
        if number in dictonary:
            dictonary[number].append(index)
        else:
            dictonary[number] = [index]
        if len(dictonary[number]) > n:
            for i in dictonary[number]:
                delete_indexes.append(i)
    return [j for i, j in enumerate(data) if i not in delete_indexes]


print(solution([1,2,3], 0))
print(solution([120,120,1,1,1,1], 5))
print(solution([1, 2, 2, 3, 3, 3, 4, 5, 5], 1))
print(solution([1, 1, 1, 1, 1, 2, 2, 3, 3, 3, 4, 5, 5], 2))
